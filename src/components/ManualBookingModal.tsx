...
interface ClosureDateEntry { closureDate: string; reason?: string; }
...
interface ManualBookingModalProps {
    ...
    lockedEmployeeId?: string;  // Çalışan kendi adına randevu oluştururken
    onSuccess?: () => void;
}
...
function mergeBookedSlots(slots: { startTime: string; endTime: string }[]): { start: number; end: number }[] {
    ...
    for (let i = 1; i < ranges.length; i++) {
        if (ranges[i].start <= cur.end) cur.end = Math.max(cur.end, ranges[i].end);
...
export const ManualBookingModal: React.FC<ManualBookingModalProps> = ({
    ...
    // Adım sırası: lockedEmployeeId varsa personel adımını atla
    const STEPS: { id: Step; label: string; number: number }[] = lockedEmployeeId
        ? [
            { id: 'customer', label: 'Müşteri', number: 1 },
    ...
    const [searchResults, setSearchResults] = useState<import('../types/shop').ShopCustomerDto[]>([]);
    const [searching, setSearching] = useState(false);
    const [selectedCustomer, setSelectedCustomer] = useState<import('../types/shop').ShopCustomerDto | null>(null);
    ...
    const [employeeLeaveDates, setEmployeeLeaveDates] = useState<string[]>([]);
    const [bookingSuccess, setBookingSuccess] = useState<{ employeeName: string; date: string; time: string } | null>(null);
    ...
    useEffect(() => {
        if (customerMode !== 'registered' || debouncedSearch.trim().length < 2) {
            setSearchResults([]);
            return;
        }
        setSearching(true);
        import('../api/shop.service').then(({ shopService }) => {
            shopService.getShopCustomers(shopId, debouncedSearch)
                .then(setSearchResults)
                .catch(() => setSearchResults([]))
                .finally(() => setSearching(false));
        });
    }, [debouncedSearch, customerMode, shopId]);
    ...
    const timeSlots = useMemo(() => {
        ...
        for (let slotStart = workStart; slotStart < workEnd; slotStart += 15) {
            const slotWindowEnd = slotStart + 15;
            const slotServiceEnd = slotStart + totalDuration;
            const isPast = isToday && slotStart < nowMinutes;
            const isBreak = breakStart !== null && breakEnd !== null && slotStart < breakEnd && slotWindowEnd > breakStart;
            const isBooked = bookedRanges.some(b => slotStart < b.end && slotWindowEnd > b.start);
            slots.push({ time: minutesToHHMM(slotStart), endTime: minutesToHHMM(slotServiceEnd), isBooked, isBreak, isPast });
        }
        return slots;
    ...
    const availableDays = useMemo(() => (
        Array.from({ length: bookingDaysAhead }, (_, i) =>
    ...
    const handleSlotClick = (time: string) => {
        ...
        if (endMin > wEnd) {
            setSelectionError(`${time}'da başlayan ${totalDuration} dk'lık hizmet çalışma bitiş saatini (${minutesToHHMM(wEnd)}) aşıyor.`);
            setSelectedTime('');
            return;
        }
        if (availability.breakStartTime && availability.breakEndTime) {
            ...
            if (startMin < bEnd && endMin > bStart) {
                setSelectionError(`Bu saat aralığı mola zamanıyla çakışıyor (${minutesToHHMM(bStart)}–${minutesToHHMM(bEnd)}).`);
                setSelectedTime('');
                return;
            }
        }
        ...
        if (conflict) {
            setSelectionError(`Bu saat aralığı mevcut bir randevuyla çakışıyor (${minutesToHHMM(conflict.start)}–${minutesToHHMM(conflict.end)}).`);
            setSelectedTime('');
            return;
        }
        setSelectionError(null);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setBookingSuccess({
                ...
                time: `${selectedTime} – ${minutesToHHMM(hhmmToMinutes(selectedTime) + totalDuration)}`,
            });
            onSuccess?.();
        } catch (error: any) {
    ...
    const nextStep = () => {
        ...
        if (currentStep === 'customer') {
            if (customerMode === 'named') {
                ...
                // Set name and phone to match selected customer to be passed in handleSubmit
            }
    ...
    const prevStep = () => {
        ...
        const idx = stepOrder.indexOf(currentStep);
        if (idx > 0) {
            if (currentStep === 'datetime') setSelectedTime('');
    ...
    const noAvailableSlots = availability?.isWorking && !loading && timeSlots.every(s => s.isBooked || s.isBreak || s.isPast);
            ...
            {selectionError && (
                <div className="fixed inset-0 flex items-center justify-center z-[70] pointer-events-none">
                    <div className="pointer-events-auto mx-4 max-w-sm w-full bg-red-50 border border-red-200 rounded-2xl shadow-2xl p-4 flex items-start gap-3">
                        ...
                        <button onClick={() => setSelectionError(null)} className="shrink-0 p-1 rounded-lg hover:bg-red-100 text-red-400 hover:text-red-600">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                </div>
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 z-50">
                <div className="bg-white w-full sm:rounded-2xl sm:max-w-xl max-h-[95vh] sm:max-h-[90vh] flex flex-col rounded-t-2xl shadow-2xl">
                    ...
                    <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 shrink-0">
                        <div>
                            <h2 className="text-lg font-bold text-gray-900">Manuel Randevu</h2>
                            <p className="text-xs text-gray-400 mt-0.5">Kapıdan gelen veya telefon randevusu</p>
                        </div>
                    ...
                    <div className="px-4 pt-4 pb-3 border-b border-gray-100 shrink-0">
                        <div className="flex items-center justify-between relative">
                            <div className="absolute top-4 left-4 right-4 h-0.5 z-0">
                                ...
                                <div className="absolute left-0 top-0 h-full bg-primary-500 transition-all duration-500"
                            ...
                            {STEPS.map((step, i) => {
                                ...
                                return (
                                    <div key={step.id} className={`flex flex-col gap-1.5 z-10 ${isFirst ? 'items-start' : isLast ? 'items-end' : 'items-center'}`}>
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all duration-300 ${isDone ? 'bg-primary-500 border-primary-500 text-white' :
                                                isActive ? 'bg-white border-primary-500 text-primary-600 shadow-md' :
                                                    'bg-white border-gray-200 text-gray-400'
                                            }`}>
                                            {isDone ? <Check className="w-4 h-4" /> : step.number}
                                        </div>
                                        <span className={`text-[10px] sm:text-xs font-semibold whitespace-nowrap ${isActive ? 'text-primary-600' : isDone ? 'text-primary-400' : 'text-gray-400'
                                            }`}>{step.label}</span>
                                    </div>
                                );
                            })}
                        ...
                        {bookingSuccess && (
                            <div className="flex flex-col items-center justify-center py-12 px-6 text-center space-y-6">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-black text-gray-900">Randevu Oluşturuldu!</h3>
                                    <p className="text-gray-500 text-sm">Manuel randevu başarıyla kaydedildi.</p>
                                </div>
                            ...
                            {currentStep === 'customer' && (
                                <div className="p-5 space-y-4">
                                    <p className="text-sm font-semibold text-gray-500 uppercase tracking-wide flex items-center gap-2">
                                        <User className="w-4 h-4" /> Müşteri Bilgisi
                                    </p>
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        ...
                                        >
                                            <BookUser className={`w-7 h-7 ${customerMode === 'registered' ? 'text-primary-600' : 'text-gray-400'}`} />
                                            <div className="text-center">
                                                <p className={`font-bold text-sm ${customerMode === 'registered' ? 'text-primary-900' : 'text-gray-700'}`}>Kayıtlı</p>
                                                <p className="text-[11px] text-gray-400 mt-0.5">Geçmiş müşteri</p>
                                            </div>
                                    ...
                                    {/* Kayıtlı Müşteri Formu */}
                                    {customerMode === 'registered' && (
                                        <div className="space-y-3 pt-1 relative">
                                            {selectedCustomer ? (
                                                <div className="p-4 bg-primary-50 border-2 border-primary-500 rounded-xl flex items-center justify-between">
                                                    <div>
                                                        <p className="font-bold text-primary-900">{selectedCustomer.name}</p>
                                                        {selectedCustomer.phone && <p className="text-sm text-primary-600 mt-0.5">{selectedCustomer.phone}</p>}
                                                    </div>
                                                    <button 
                                                        onClick={() => {
                                                            setSelectedCustomer(null);
                                                ...
                                                <div>
                                                    <label className="block text-xs font-semibold text-gray-600 mb-1.5">
                                                        Kayıtlı Müşteri Ara <span className="text-red-500">*</span>
                                                    </label>
                                                    <input
                                                        type="text"
                                                        value={searchTerm}
                                                    ...
                                                    {searchTerm.trim().length >= 2 && (
                                                        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 shadow-xl rounded-xl max-h-60 overflow-y-auto z-10 p-2">
                                                            {searching ? (
                                                                <p className="text-center text-sm text-gray-400 py-4">Aranıyor...</p>
                                                            ) : searchResults.length > 0 ? (
                                                                searchResults.map((customer, idx) => (
                                                                    <button
                                                                        key={customer.userId || customer.phone || idx}
                                                                        onClick={() => {
                                    ...
                                    {loading ? (
                                        ...
                                        return (
                                            <button key={emp.id} onClick={() => setSelectedEmployeeId(emp.id)}
                                                className={`w-full flex items-center gap-4 py-3 pr-4 pl-3 rounded-2xl border-2 transition-all text-left ${selected ? 'border-primary-500 bg-primary-50' : 'border-gray-100 bg-white hover:border-gray-300'
                                                    }`}>
                                                <div className="h-14 w-14 min-w-[56px] aspect-square rounded-full overflow-hidden shrink-0 bg-primary-100 flex items-center justify-center">
                                                    {emp.imageUrl ? (
                                                ...
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 border-2 transition-all ${selected ? 'bg-primary-500 border-primary-500' : 'border-gray-300'
                                                    }`}>
                                ...
                                <div className="p-5">
                                                    ...
                                                    <div key={cat.id}>
                                                        ...
                                                        <div className="space-y-2">
                                                            {cat.services.map(service => {
                                                                ...
                                                                return (
                                                                    <div key={service.id} className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl border-2 transition-all ${isSelected ? 'border-primary-400 bg-primary-50' : 'border-gray-100 bg-white hover:border-gray-200'
                                                                        }`}>
                                                                        <div className="flex-1 min-w-0">
                                                                            <p className={`font-semibold text-sm leading-tight ${isSelected ? 'text-primary-900' : 'text-gray-900'}`}>{service.name}</p>
                                                                        ...
                                                                        <div className="flex items-center gap-1.5 shrink-0">
                                                                            <button onClick={() => removeService(service.id)} disabled={count === 0}
                                                                                className={`w-7 h-7 rounded-full flex items-center justify-center border-2 transition-all ${count > 0 ? 'border-primary-500 text-primary-600 hover:bg-primary-50' : 'border-gray-200 text-gray-300 cursor-not-allowed'
                                                                                    }`}>
                                                                                <Minus className="w-3.5 h-3.5" />
                                                                            </button>
                                                                            <span className={`w-5 text-center font-bold text-sm ${count > 0 ? 'text-primary-700' : 'text-gray-400'}`}>{count}</span>
                                                                            <button onClick={() => addService(service)}
                                    ...
                                    {selectedServices.length > 0 && (
                                        <div className="mt-4 p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between">
                                            <div><p className="text-xs text-gray-500 font-medium">Toplam Süre</p><p className="font-bold text-primary-700 text-sm">{totalDuration} dk</p></div>
                                            <div className="text-right"><p className="text-xs text-gray-500 font-medium">Toplam Tutar</p><p className="font-bold text-lg text-primary-700">₺{totalPrice}</p></div>
                                        </div>
                                    )}
                                </div>
                            )}
                                    ...
                                    <div>
                                        ...
                                        <div ref={scrollRef} className="flex gap-2 overflow-x-auto scrollbar-hide pb-1 select-none">
                                            {availableDays.map(dateStr => {
                                                ...
                                                return (
                                                    <button key={dateStr} disabled={!!closedReason} onClick={() => setSelectedDate(dateStr)}
                                                        className={`flex flex-col items-center shrink-0 w-[72px] py-2 px-1 rounded-2xl border-2 transition-all ${closedReason ? 'bg-red-50 border-red-200 cursor-not-allowed' :
                                                ...
                                                <div className="grid grid-cols-4 gap-2">
                                                    {(() => {
                                                        ...
                                                        return timeSlots.map(({ time, endTime, isBooked, isBreak, isPast }) => {
                                                            ...
                                                            let slotClass = '';
                                                            if (isSelected) slotClass = 'bg-primary-600 text-white border-primary-600 shadow-md scale-105';
                                                            else if (isInRange) slotClass = 'bg-primary-600 text-white border-primary-600';
                                                            else if (isPast) slotClass = 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed';
                                                            else if (isBooked) slotClass = 'bg-blue-50 text-blue-300 border-blue-200 cursor-not-allowed';
                                                            else if (isBreak) slotClass = 'bg-amber-50 text-amber-400 border-amber-200 cursor-not-allowed';
                                                            else slotClass = 'bg-white text-gray-700 border-gray-200 hover:border-primary-400 hover:bg-primary-50';
                                                            return (
                                                                <button key={time} disabled={disabled} onClick={() => handleSlotClick(time)}
                                                                    className={`py-2.5 text-xs font-semibold rounded-xl border transition-all duration-150 ${slotClass}`}>
                                                                    {time}
                                                                    {isSelected && <span className="block text-[9px] text-primary-200">–{endTime}</span>}
                                                                </button>
                                                            );
                                                        });
                                                    })()}
                                ...
                                <div className="p-5 space-y-4">
                                    ...
                                    <div className="bg-gray-50 rounded-2xl border border-gray-100 divide-y divide-gray-100 overflow-hidden">
                                        ...
                                        <div className="px-4 py-3 flex justify-between items-center text-sm">
                                            <span className="text-gray-500">Tarih</span>
                                            <span className="font-bold text-gray-900">{format(new Date(`${selectedDate}T12:00:00`), 'd MMMM yyyy, EEEE', { locale: tr })}</span>
                                        </div>
                                        <div className="px-4 py-3 flex justify-between items-center text-sm">
                                            <span className="text-gray-500">Saat</span>
