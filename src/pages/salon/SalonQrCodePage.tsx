        ...
        <div className="max-w-2xl mx-auto px-4 py-10">
            ...
            {isModalOpen && (
                ...
                >
                    ...
                    {/* Modal card — max-h + flex-col so inner div scrolls */}
                    <div className="relative z-10 w-full max-w-md bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90dvh]">

                        {/* Close — absolute on outer non-scrolling card, always visible */}
                        <button
                            onClick={() => setIsModalOpen(false)}
                            className="absolute top-3 right-3 z-30 w-8 h-8 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/30 text-white transition-colors"
                        >
                            <X className="w-4 h-4" />
                        </button>
                        ...
                        <div className="overflow-y-auto flex-1">
                            ...
                            {/* Gradient header */}
                            <div className="bg-gradient-to-br from-primary-700 via-primary-800 to-primary-900 px-5 pt-6 pb-5 text-white">
                                <p className="text-[10px] font-semibold tracking-widest uppercase text-primary-200 mb-1">SalonBir</p>
                                        ...
                                        <div className="flex items-start gap-2">
                                            ...
                                            <div className="flex flex-wrap gap-1">
                                                {categoryNames.map((cat) => (
                                                    <span key={cat} className="inline-block px-2 py-0.5 bg-white/20 rounded-full text-xs font-semibold text-white">
                                                        {cat}
                                                    </span>
                                                ))}
                                            </div>
                            ...
                            {/* QR Code */}
                            <div className="bg-gray-50 flex flex-col items-center py-5 px-5">
                                <div className="bg-white p-3 rounded-2xl shadow-md ring-1 ring-gray-100">
                                    <QRCodeCanvas
                                        ref={canvasRef}
                                        value={shopUrl}
                                        size={200}
                                ...
                                <p className="mt-2 text-[10px] text-gray-400 text-center font-mono break-all px-2 max-w-xs">{shopUrl}</p>
                            </div>

                            {/* Download buttons */}
                            <div className="bg-white px-5 pb-5 pt-2 space-y-2">
                                <button
                                    onClick={handlePrintFlyer}
                                    className="w-full flex items-center justify-center gap-2 py-3 bg-primary-700 hover:bg-primary-800 text-white font-bold text-sm rounded-xl transition-all duration-200 active:scale-[0.98] shadow-sm shadow-primary-700/20"
                                >
