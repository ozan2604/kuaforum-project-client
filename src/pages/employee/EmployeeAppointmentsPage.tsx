...
export const EmployeeAppointmentsPage: React.FC = () => {
    ...
    const totalRevenue   = dayAppointments.filter(a => a.status === AppointmentStatus.Completed).reduce((s, a) => s + a.price, 0);
                                ...
                                <div className="relative" style={{ height: `${visibleSlotCount * SLOT_HEIGHT}px` }}>
                                            ...
                                            >
                                                <span
                                                    className={`absolute left-2 top-1 text-xs tabular-nums select-none ${
                                                        isHourMark ? 'font-bold text-gray-600' : 'text-gray-300 text-[10px]'
                                                    }`}
                                                >
                                    ...
                                    {dayAppointments.map(app => {
                                        ...
                                        return (
                                            <div
                                                key={app.id}
                                                title={`${app.customerName} • ${app.serviceName} • ${format(new Date(app.startTime), 'HH:mm')}–${format(new Date(app.endTime), 'HH:mm')}`}
