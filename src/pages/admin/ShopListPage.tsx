...
export const ShopListPage: React.FC = () => {
    ...
    const fetchShops = async () => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || 'Salonlar yüklenemedi.');
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => { fetchShops(); }, [page, debouncedSearchTerm]);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">Kayıtlı Salonlar</h2>
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Konum & İletişim</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sahip</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Durum & Tarih</th>
                                            ...
                                            <div className="text-xs text-gray-500 flex items-center gap-2 mt-1">
                                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800">
                                                    ...
                                                    {shop.categories?.length > 0
                                                        ? shop.categories.slice(0, 2).map((c: number) => ShopCategoryLabels[c as ShopCategory]).join(', ')
                                                        : 'Diğer'}
                                                </span>
                ...
                {shops.length === 0 && !loading && (
                    <div className="flex flex-col items-center justify-center p-12 bg-white border-t border-gray-200">
                        ...
                        <p className="text-lg text-gray-500">Salon bulunamadı.</p>
                    </div>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/50">
                    {showPasswordModal ? (
                        <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 text-center">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-end gap-3 mt-4">
                                    <button
                                        onClick={() => { setShowPasswordModal(false); setShopToDelete(null); }}
                        ...
                        <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
                            <h3 className="text-xl font-bold text-gray-900 mb-3">Salonu Sil</h3>
                            <p className="text-gray-600 mb-6">
                                <strong>"{shopToDelete.name}"</strong> isimli salonu silmek istediğinize emin misiniz?
                                Bu işlem geri alınamaz; salonla ilişkili tüm veriler (hizmetler, çalışanlar, randevular) kalıcı olarak silinecektir.
                            </p>
                            <div className="flex justify-end gap-3">
                                ...
                                <button
                                    onClick={() => executeDelete(shopToDelete.id)}
                                    disabled={deleteLoading}
                                    className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50"
                                >
